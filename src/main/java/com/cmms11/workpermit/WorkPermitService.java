...
public class WorkPermitService {
    ...
    public Page<WorkPermitResponse> list(String permitId, String plantId, String jobId, String status, String stage, String plannedDateFrom, String plannedDateTo, Pageable pageable) {
    ...
    public WorkPermitResponse get(String permitId) {
        WorkPermit workPermit = getExisting(permitId);
        List<WorkPermitItem> items = itemRepository.findByPermit(
            MemberUserDetailsService.DEFAULT_COMPANY,
            permitId
        );
        return WorkPermitResponse.from(workPermit, items);
    }
    ...
    public WorkPermitResponse create(WorkPermitRequest request) {
        ...
        // ⭐ 신규 생성 시 초기 상태 설정
        // request에서 stage가 전달되면 우선 사용 (현재는 PLN만 허용)
        // 전달되지 않으면 기본값 "PLN" 사용
